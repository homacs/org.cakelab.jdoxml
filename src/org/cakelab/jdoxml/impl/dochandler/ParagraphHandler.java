...
public class ParagraphHandler extends BaseHandler<ParagraphHandler> implements IDocPara {
	...
	public static IDocPara join(List<ParagraphHandler> paragraphs) {
		if (paragraphs.size() == 0) {
			return null;
		} else if (paragraphs.size() == 1) {
			return paragraphs.get(0);
		} else {
			IDocParaJoined joined = new IDocParaJoined();
			for (ParagraphHandler doc : paragraphs) {
				joined.addAll(doc.m_children);
			}
			return joined;
		}
	}
